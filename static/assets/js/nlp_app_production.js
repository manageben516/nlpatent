...
$( document ).ready(function() {
	...
	// ---------------------------------------------------------------
	// duplicate key elements field in step 1
	// ---------------------------------------------------------------
	...
	var nlp_dataroute = {
		...
		step1_submit 				: "https://www.nlpatent.com/submitstep1data",
		step2b_submit 				: "https://www.nlpatent.com/submitstep2data",
		...
		step1_load 				: "https://www.nlpatent.com/getstep1data",
		step2a_load 				: "https://www.nlpatent.com/getstep2classes",
		step2b_load 				: "https://www.nlpatent.com/getstep2subclasses",
		step3_results_load 			: "https://www.nlpatent.com/getstep3results",
		...
		apply_filters_submit 		: "https://www.nlpatent.com/applyfilters",
		update_results_submit 		: "https://www.nlpatent.com/updateresults",
		clearupdate_results_submit 	: "https://www.nlpatent.com/clearupdated",
	...
	function nlpapp_load_stepone_complete(data,callback){
		...
		if( data.key_elements ){
			...
			for(i=0;i<data.key_elements.length;i++){
				if (i == 0){
					$("#element_1","#nlpapp_step_one_form").val( data.key_elements[i] );
				} else {
					$("#keyelement-holder","#nlpapp_step_one_form").append("<div class='form-group'><div class='keyelement-row flex-row top' id='keyelement-row-" + i + "'><div class='keyelement-row-field-holder'><input type='text' class='form-control keyelement-field' id='element_" + i +"' name='element_" + i +"' value='" + data.key_elements[i] + "'><div class='help-block with-errors'></div></div><div><div class='hex-button-small nlp-bluegray remove-keyelement'><button type='button' class='btn btn-primary'><div class='icon nlpapp-icon-minus'></div></button></div></div></div></div>");
				}
			}
			if(data.key_elements.length == 5){
				$("#duplicate-keyelement","#nlpapp_step_one_form").addClass("disabled");
			}
		}
	...
	// ---------------------------------------------------------------
	function nlpapp_load_steptwo_a(){
		nlapp_reset_ui();
		ajaxCall( nlp_dataroute.step2a_load, "GET", "", nlpapp_load_steptwo_a_complete, "application", null, "ASSESSING INPUT... THIS MIGHT TAKE A MINUTE");
	}
	...
	function nlpapp_load_stepthree(){
		...
		$("#nlpapp-filters-notification").html("");
		$("#nlpapp-filters").hide();
		$("#nlpapp_hidefilters").parent().hide();
		$("#nlpapp_showfilters").parent().show();
	...
	function nlpapp_load_stepthree_results(relay){
		...
		if (relay && relay == "relay_true"){
			ajaxCall( nlp_dataroute.step3_results_load, "POST", "", nlpapp_load_stepthree_results_complete, "application", null, "PROCESSING...");
		} else {
			ajaxCall( nlp_dataroute.step3_results_load, "POST", "", nlpapp_load_stepthree_results_complete, "application", null, "RETRIEVING PRIOR ART...");
		}
	}
	function nlpapp_load_stepthree_results_complete(data,callback){
		...
		if(data.prior_art_results == null && data.heading == "NO RESULTS"){
		    $("tbody","#prior-art-results-table").html("<tr class='empty-row-message' style='display:table-row;'><td colspan='4' align='center'><br>You must perform a search.<br><br></td></tr>");
		} else {
		...
		if ( data.filters ){
    		...
    		//}
    		filter_key_elements= "<div id='nlpapp_stepthree_filters_keyelements_holder'><div class='row filters-row' style='display: flex;align-items: top;'><div class='col-xs-1'><div class='hex-button-helper'><button type='button' class='btn btn-primary' data-toggle='tooltip' data-placement='top' title='Sort by specific key words or phrases in the prior art.'>?</button></div></div><div class='col-xs-11'><span id='keyelement-holder'><div class='form-group'><label for=''>Key Elements</label><div class='keyelement-row flex-row top'><div class='keyelement-row-field-holder'><input type='text' class='form-control keyelement-field' id='element_1' name='element_1' style='margin-top:.25em;margin-bottom:.25em;'><div class='help-block with-errors'></div></div><div><div class='hex-button-small nlp-orange' id='duplicate-keyelement' style='margin-top:.25em;margin-bottom:.25em;'><button type='button' class='btn btn-primary'><div class='icon nlpapp-icon-plus'></div></button></div></div></div></div></span></div></div></div>";
    		...
    		$("#nlpapp-filters-form").prepend("<div class='form-feedback'></div>").prepend(filter_dates).prepend(filter_cpc_subclasses).prepend(filter_result_elements).prepend(filter_key_elements);
    		...
    		if ( filter_result_elements !== ""){
    			$("#result_elements","#nlpapp-filters-form").multiselect({columns: 2,placeholder: "Result Elements",search: false,selectAll: false});
    		}
	...
	function nlpapp_load_stepthree_results_filtered_complete(data,callback){
		...
		if(data.prior_art_results){
			for(i=0;i<data.prior_art_results.length;i++){
				...
				$("tbody","#prior-art-results-table").append("<tr data-id='" + data.prior_art_results[i].p + "' data-text='" + data.prior_art_results[i].T + "' data-cpc='" + data.prior_art_results[i].c + "'><td width='65px' scope='row'><button type='button' class='nlpapp-button nlpapp-priorart-delete"+disabled_flag+"'><div class='icon nlpapp-icon-delete'></div></button><button type='button' class='nlpapp-button nlpapp-priorart-save"+disabled_flag+"'><div class='icon nlpapp-icon-plus'></div></button></td><td width='100px'><a href='https://patents.google.com/patent/" + data.prior_art_results[i].p + "' target='_blank'>" + data.prior_art_results[i].p + "</a></td><td width='*'  valign='middle'>" + data.prior_art_results[i].t + "</td><td width='50px' align='right' valign='middle'><var>" + data.prior_art_results[i].c + "</var></td></tr>");
				// show filtered info
				...
				if ( data.prior_art_results[i].k){
					key_elements_filter = "Key Elements :";
				...
				if ( data.prior_art_results[i].r){
					result_elements_filter = "Result Elements :";
	...
	// ---------------------------------------------------------------
	$(document.body).on("click", "#nlpapp-download-analysis", function(){
		ajaxCall( nlp_dataroute.step3_savedresults_load, "GET", "", nlpapp_download_analysis_complete, "#nlpapp-analysis-save", null, "PREPARING SUMMARY...");
	});
	...
	$(document.body).on("click", "#nlpapp-filters-apply", function(){
		if ( !$(this).hasClass("disabled")){
		    ...
		    if ( validate_step_three_filters_form === true ){
			    ...
			    var key_elements_data="",results_elements_data="",cpc_sublcasses_data="",from_date_data="",to_date_data="";
			    var key_elements_data_temp="",results_elements_data_temp="",cpc_sublcasses_data_temp="";
			    ...
			    if ( $("#result_elements","#nlpapp-filters-form").length ){
			    	...
			    	if(!results_elements_data_temp==""){
			    		...
			    		filters_notification = filters_notification + "Result Elements : " + results_elements_data_temp.join(", ") + "<br>";
			    	}
			    }
			    ...
			    var form_data = key_elements_data+"result_elements="+results_elements_data+"&cpc_subclasses="+cpc_sublcasses_data+"&date_from="+from_date_data+"&date_to="+to_date_data;
			    if( form_data == "result_elements=&cpc_subclasses=&date_from=&date_to="){
			    	$("#nlpapp-filters-notification").html("");
			    	// reload
			    	nlpapp_load_stepthree_results();
			    } else {
			    	$("#nlpapp-filters-notification").html(filters_notification);
	...
	$(document.body).on("click", ".nlpapp-priorart-save", function(){
		if ( !$(this).hasClass("disabled")){
			...
			ajaxCall( nlp_dataroute.step3_save_result , "POST", form_data, nlpapp_stepthree_saveresult_complete, "application", null, "PROCESSING...");
			check_priorart_table("prior-art-results-table");
		}
	});
	...
	nlp_router.hooks({
   	...
   	function nlpapp_render_tabs(){
		...
		var project_tab_ids = ["-step1","-step1","-step2a","-step3","-step4"];
